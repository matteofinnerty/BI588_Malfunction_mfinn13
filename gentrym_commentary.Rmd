...
my comments in the code chunks are marked off by == equal signs on either side ==
...
# == i had forgotten how to do this, thank you! ==
...
# == instead of having your program redo the sqrt(pStar * ...) ==
# == that you use in the se assignment above, you could just   ==
# == use the "se" object and save some memory and time         ==
   
   zScore <- (p2 - p1)/(sqrt( pStar * (1 - pStar) * ((1/n1)+ (1/n2)))) #calculates z score
   
# == I think you did this right because I did it the same way  ==
# == this does calculate the p value, so no worries            ==
   
   p <- 2*pnorm(abs(zScore), lower.tail = F) #calculates p value? I'm not sure I did this right. I think this works. 
   
# == if you're ever doing this in the future, you can use one  ==
# == line to calculate both the upper and lower CI like this:  ==
# == CI <- (p1-p2) + c(-1, 1) * qnorm(1-(1-conf.level)/2) * se ==
# == This creates an list CI that stores  both values          ==
...
# == It's usually better to run checks like these first bc w/  ==
# == some functions a parameter that doesnt pass the check can ==
# == break the function, but it doest seem to matter here      ==
...
```{r}
# == you already loaded the package libraries above, ==
