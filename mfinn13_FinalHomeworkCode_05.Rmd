...
Write a simple R function, Z.prop.test(), that can perform one- or two-sample Z-tests for proportion data, using the following guidelines: Your function should take the following arguments: p1 and n1 (no default) representing the estimated proportion and sample size (i.e., based on your sample data); p2 and n2 (both defaulting to NULL) that contain a second sample’s proportion and sample size data in the event of a two-sample test; p0 (no default) as the expected value for the population proportion; and alternative (default “two.sided”) and conf.level (default 0.95), to be used in the same way as in the function t.test().
...
Z.prop.test <- function(p1, n1, p2 = NULL, n2 = NULL, p0, alternative = "two.sided", conf.level = 0.95){
  ...
  if (alternative == "two.sided")  #alternative hypothesis: p1 does not equal p2
  {
   ...
   zScore <- ((p2 - p1)/se) #calculates z score
   
   p <- 2*pnorm(abs(zScore), lower.tail = F) #calculates p value? I'm not sure I did this right. I think this works. 
   
   CIlower <- (p2-p1) - qnorm(conf.level+((1-conf.level)/2))*se #calculates confidence interval lower bound
   CIupper <- (p2-p1) + qnorm(conf.level+((1-conf.level)/2))*se #calculates confidence interval upper bound
   ...
   if( ((n1*p1) <= 5) | ((n2*p2) <= 5))
    {
      warning("assumption np > 5 is not met. ")
   }
   if ( (n1*(1-p1)) <= 5 | (n2*(1-p2)) <= 5)
   {
     warning("assumption n(1-p) > 5 is not met. ")
   }
...
Z.prop.test(p1 = 0.50, n1 = 10000, p2 = 0.48, n2 = 10000, alternative = "two.sided", conf.level = 0.95)
...
plot2 <- ggplot(data = df_PIandCI, aes(x = brainSize, y = longevity))+
  geom_point()+
  geom_line(aes(y = C_fit, color = "Fit Line"))+ #fit line
  geom_line(aes(y = C_lwr, color = "CI"))+ #CI lower 
  geom_line(aes(y = C_upr, color = "CI"))+ #CI upper
  geom_line(aes(y = P_lwr, color = "PI"))+ #PI lower
  geom_line(aes(y = P_upr, color = "PI"))+ #PI upper
  labs(x = "Mean Species Brain Size", y = "Max Longevity", title = "Max   Longevity vs. Mean Species Brain Size")+
  scale_color_manual(values = c("Fit Line" = "black", "CI" = "blue", "PI" = "red"))+
  theme_bw()
...
plot4 <- ggplot(data = df_PIandCI_2, aes(x = logBrainSize, y = logLongevity))+
  geom_point()+
  geom_line(aes(y = C_fit2, color = "Fit Line"))+ #fit line
  geom_line(aes(y = C_lwr2, color = "CI"))+ #CI lower 
  geom_line(aes(y = C_upr2, color = "CI"))+ #CI upper
  geom_line(aes(y = P_lwr2, color = "PI"))+ #PI lower
  geom_line(aes(y = P_upr2, color = "PI"))+ #PI upper
